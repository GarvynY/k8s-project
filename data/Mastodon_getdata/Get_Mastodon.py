...
def get_location_info(account):
    ...
    # 从自我介绍字段尝试提取
    note = BeautifulSoup(account.get('note', ''), 'html.parser').get_text()
    location_keywords = [
        'Sydney', 'Melbourne', 'Brisbane', 'Canberra', 'Perth', 'Adelaide',
        'Hobart', 'Darwin', 'Gold Coast', 'Newcastle', 'Wollongong', 'Cairns',
        'Sunshine Coast', 'Geelong', 'Townsville', 'Ballarat', 'Bendigo', 'Launceston',
        'Mackay', 'Bundaberg', 'Toowoomba', 'Rockhampton', 'Tamworth', 'Albany',
        'Port Macquarie', 'Alice Springs', 'Mount Isa', 'Kalgoorlie', 'Shepparton',
        'Wagga Wagga', 'Whyalla', 'Dubbo', 'Broken Hill'
    ]
    ...
    # 从自定义字段中尝试提取
    if not location and 'fields' in account:
        for field in account['fields']:
            field_name = field.get('name', '').lower()
            field_value = BeautifulSoup(field.get('value', ''), 'html.parser').get_text()
            if any(kw.lower() in field_value.lower() for kw in location_keywords):
                location = field_value
                break

    return location, None  # 仍无 geolocation 坐标
# 3) 抓取函数（分页 + 时间过滤）
def fetch_keyword(keyword, SINCE_DT, PAGE_LIMIT=20):
    ...
    while total_fetched < 500:  # 只抓取 300 条数据
