...
# -*- coding: utf-8 -*-
"""
mastodon_election_300_filtered_with_fallback.py

抓取 Mastodon 上 #ausvotes 的 300 条帖子，按发布时间倒序，
提取字段并保存到 CSV。解析不到城市名时随机分配，
并确保 location 只包含已知澳洲城市（不含州名/Australia）。
...
- emotion_label
- location        （澳洲主要城市之一）
- geolocation     （对应城市的 lat,lon）

依赖：
    pip install Mastodon.py vaderSentiment geopy

用法：
    python mastodon_election_300_filtered_with_fallback.py
"""
import re
import time
import csv
...
HASHTAG      = "ausvotes"
OUTPUT_CSV   = "mastodon_election_weekly.csv"
PAGE_SIZE    = 40
...
def fetch_posts_for_week(start_date: datetime, end_date: datetime):
    ...
    while True:
        print(f"Page {page}: fetching, collected {len(rows)}")
...
if __name__ == "__main__":
    ...
    print(f"Fetching posts from {one_week_ago.date()} to {today.date()} ...")
    weekly_data = fetch_posts_for_week(one_week_ago, today)
    save_to_csv_append(weekly_data, OUTPUT_CSV)
